...
const wrapStyleImports = function (data, isDev, mapping) {
  ...
  if (!isDev) {
    data.links = `${buildLink(mapping['/fas-sys/dist/vendor.css'])}${buildLink(mapping['/fas-sys/dist/'+data.name + '.css'])}`;
  }
};
/**
...
const wrapScriptImports = function (data, isDev, mapping) {
  ...
  if (isDev) {
    data.scripts += `${buildScriptFullUrl('/fas-sys/dll/vendor.dll.js')}`;
    data.scripts += `${buildScriptFullUrl('/fas-sys/dist/' + data.name + '.bundle.js')}`;
  } else {
    data.scripts += `${buildScript(mapping['/fas-sys/dist/vendor.js'])}`;
    data.scripts += `${buildScript(mapping['/fas-sys/dist/'+data.name+ '.js'])}`;
  }
};
/**
 * 设置通用的响应头,页面不缓存！！
...
const renderIndex = function (req, res, next) {
  res.redirect('/fas-sys/index');
};

function addRoute(app, options) {
  app.get('/fas-sys/index**',function (req, res, next) {
  ...
  app.get('/fas-sys/crowdFunding**',function (req, res, next) {
    renderTemplateSync(req,res,{
      title:'众筹管理',
      name:'crowdFunding',
      scripts:""
