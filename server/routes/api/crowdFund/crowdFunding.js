...
router.post('/getRecord', (req, res, next) => {
    const {projectId,merchantId,belongType} = req.body;
    service.getRecord(req, {
        projectId,merchantId,belongType
    }).then((body) => {
        res.json(body);
    }).catch((err) => {
        next(err);
...
router.post('/deleteProject', (req, res, next) => {
    //logger.info(req.body);
    const {projectId,belongType,merchantId} = req.body;
    service.deleteProject(req, {
        projectId,belongType,merchantId
    }).then((body) => {
        res.json(body);
    }).catch((err) => {
        next(err);
...
router.post('/offlineProject', (req, res, next) => {
    const {projectId,belongType,merchantId,projectShowStatus} = req.body;
    service.offlineProject(req, {
        projectId,belongType,merchantId,projectShowStatus
    }).then((body) => {
        res.json(body);
    }).catch((err) => {
        next(err);
